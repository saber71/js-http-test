...
export class ExpectResponse<Data> {
  ...
  /* 设置期待的响应头 */
  expectHeader(key: keyof AxiosResponse["headers"], expectValue: string | RegExp) {
  ...
  async done() {
    ...
    if (typeof this._expectStatus === "number")
      expect(this._buildStatusObject(res.status)).toEqual(this._buildStatusObject(this._expectStatus))
  ...
  /* 测试字符串是否满足期待 */
  private _toBe(_: Response<Data>, value: string, expectValue: string | RegExp) {
...
/* 待测试的Response对象类型 */
export interface Response<Data> extends Pick<AxiosResponse<Data>, "headers" | "data" | "status"> {
  href: string
}
